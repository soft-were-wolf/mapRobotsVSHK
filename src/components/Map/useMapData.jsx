...
const useMapData = () => {
  ...
  const fetchMapsData = useCallback(async () => {
    try {
      const response = await axios.get('http://10.1.242.61/proxy/vshk/v1/maps', { headers: { token: userToken }, timeout: 5000 });
  ...
  const fetchData = useCallback(async (endpoint, setData, mapId) => {
    try {
      const response = await axios.post(`http://10.1.242.61/proxy/vshk/v1/${endpoint}`, { map_id: mapId }, { headers: { token: userToken }, timeout: 600000 });
